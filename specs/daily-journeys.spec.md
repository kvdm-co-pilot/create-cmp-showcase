...
- **JRN-01** — Given the Progress surface opens (route `progress`, HIST-01), Then its week
  section shows the last seven **logical** days
...
- **JRN-02** — Given the Profile screen, Then its weekly-stats row is a real control
  (`profile_progress_link`) whose activation opens the Progress surface — the stats' claims
  become verifiable by the surface they open, and the row's tap exists BECAUSE the
  destination does (UX-04's rule, satisfied in the other direction).
- **JRN-03** — Given the Progress surface renders, When its structure is inspected, Then it
  matches its committed golden tree (`qa/golden/progress.json`) — structural change must be
