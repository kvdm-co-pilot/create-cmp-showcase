...
class MealPlanViewModelTest {
    ...
    fun `the strip is yesterday, today and the next seven - nine days, today selected on open`() =
        runTest(dispatcher) {
            ...
            assertEquals(9, vm.stripDays.size)
            assertEquals(LocalDate(2026, 7, 21), vm.stripDays.first(), "one leading day for back-filling")
            assertEquals(today, vm.stripDays[1])
            assertEquals(LocalDate(2026, 7, 29), vm.stripDays.last())
    ...
    @Test
    fun `ticking a slot re-reads the day and cancels that slot's reminder for today`() =
        runTest(dispatcher) {
            val vm = viewModel()
            ...
            vm.state.test {
                ...
                // re-derived day, where focus has ALREADY moved on.
