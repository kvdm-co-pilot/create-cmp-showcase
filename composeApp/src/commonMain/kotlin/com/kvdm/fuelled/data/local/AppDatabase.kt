...
@Database(
    ...
    // 15: supplements stop being daily (SUPP-08) — the catalog row carries a flattened
    // schedule (kind + weekdays + cadence + anchor) and its own reminder time and ladder
    // rungs (SUPP-12); `workout_week` and `workout_done` arrive as training joins the day
    // (WORK-01), a plan you edit and a fact per logical day, kept in separate tables for the
    // reason the supplement catalog and its doses are.
    version = 15,
    exportSchema = true,
)
