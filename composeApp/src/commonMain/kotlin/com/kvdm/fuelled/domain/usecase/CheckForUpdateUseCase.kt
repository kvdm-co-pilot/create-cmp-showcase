 ...
 * latest release), then compares two versions component by component. That is why it can be tested on any JVM without
        ...
        return when (val result = repository.latestRelease()) {
            ...
            is AppResult.Success -> {
                ...
                val available = release != null &&
                    ...
                    // UPD-03: strictly greater, compared as three integers (SemVer). Equal is
                    // up to date, and an installed build that is NEWER (a local debug build) is
                    // also up to date — never a downgrade offer. An unreadable installed
                    // version offers nothing at all: we cannot claim newer than unknown.
                    installed != null && release.version > installed
                AppResult.Success(
