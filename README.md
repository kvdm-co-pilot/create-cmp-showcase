...
[![Evidence L2 — device](https://img.shields.io/badge/evidence-L2_device-26A69A)](https://github.com/kvdm-co-pilot/create-cmp) — the verify lane passed at `8745a5b` on 2026-08-27 at rung **L2 · device**. Earned by: `specCoverage`, `approvals`, `componentStories`, `reachability`, `archDoc`, `schemaHistory`, `build`, `releaseBuild`, `unitTests`, `conformance`, `goldenTrees`, `a11y`, `e2eSmoke`, `androidChecks`. The tree had 14 uncommitted files at attestation, so this describes that run, not that commit. The rung describes that run; it says nothing about changes made since.
<!-- /cmp:generated -->
